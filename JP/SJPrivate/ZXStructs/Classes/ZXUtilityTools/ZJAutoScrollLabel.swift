...
private let kTextsScrollSpace: CGFloat = ZXBOUNDS_WIDTH
private let kRepeatTimeInterval: TimeInterval = 1.0 / 40.0
private let kScrollSpeed: CGFloat = 1.5
...
public struct ZJAutoScrollAttribute
{
    ...
    public init(direction: ZJAutoScrollDirection? = nil, space: CGFloat? = nil, isPagingEnabled: Bool = false, textFont: UIFont? = nil, textColor: UIColor? = nil) {
		if  direction == nil {
			self.direction = .scollToLeft(kScrollSpeed)
		}else{
			self.direction = direction!
		}
		
		if space == nil {
			self.space  = kTextsScrollSpace
		}else{
...
class ZJAutoScrollLabel: UIView
{
    ...
    lazy var displayLink: Timer = {
        let timer = Timer.zx_scheduledTimer(timeInterval: kRepeatTimeInterval, repeats: true, completion: { [weak self] (timer) in
            self?.refreshScrollInfo()
        })
        return timer
    }()
   ...
   /// 预先计算字体宽度并缓存
    ...
    var zx_oneLoopTime: TimeInterval {
        if self.textAndWidths.count > 0 {
            if let width = self.textAndWidths.first?.value {
                let dt = width / CGFloat(1.5 / kRepeatTimeInterval) + 1
    ...
    public func resume()
    {
        displayLink.fireDate = Date()
    }
    
    public func stop()
    {
        displayLink.fireDate = Date.distantFuture
    }
}
