...
class JXGifView: UIView {
    ...
    private func createKeyFram() {
        ...
        for i in 0..<imageCount {
            ...
            if let imgRef = imageRef {
                self.imageArr.append(imgRef)
            }
            
            
            guard let sourceDict = CGImageSourceCopyPropertiesAtIndex(gifSource!, i, nil) as? Dictionary<String, Any> else {
                return
            }
            guard let gifDict = sourceDict[String(kCGImagePropertyGIFDictionary)] as? Dictionary<String, Any> else {
                return
            }
            // 每一帧的动画时间
            ...
            // 获取图片的尺寸 (适应)
            guard let imageWidth = sourceDict[String(kCGImagePropertyPixelWidth)] as? NSNumber else {
                return
            }
            guard let imageHeight = sourceDict[String(kCGImagePropertyPixelHeight)] as? NSNumber else{
                return
            }
            
            if (imageWidth.floatValue / imageHeight.floatValue) != Float(self.frame.width/self.frame.height) {
    ...
    private func fitScale(imageWidth: CGFloat, imageHeight: CGFloat) {
        ...
        if imageWidth/imageHeight > self.frame.width/self.frame.height {
            newWidth = self.frame.width
            newHeight = self.frame.width/(imageWidth/imageHeight)
        } else {
            newHeight = self.frame.height
            newWidth = self.frame.height/(imageHeight/imageWidth)
        }
...
extension JXGifView: CAAnimationDelegate {
    // Delegate 动画结束
