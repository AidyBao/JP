//
//  JXExchangTgViewController.swift
//  gold
//
...
class JXExchangTgViewController: ZXUIViewController {
    ...
    static func show(superV: UIViewController) {
        let vc = JXExchangTgViewController()
    ...
    override func viewDidLoad() {
        ...
        self.gbLB.font = UIFont.boldSystemFont(ofSize: 30)
    ...
    func jx_exchangeTg(count: String) {
        ZXHUD.showLoading(in: self.view, text: "", delay: 0)
        JXGSVAndTGManager.jx_pointExchangGSV(url: ZXAPIConst.Exchange.gsvToPoints, exVolume: count) { (succ, code, msg) in
...
extension JXExchangTgViewController: UITextFieldDelegate {
