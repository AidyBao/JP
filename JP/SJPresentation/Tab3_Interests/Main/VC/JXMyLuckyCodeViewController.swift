//
//  JXMyLuckyCodeViewController.swift
//  gold
//
...
class JXMyLuckyCodeViewController: ZXUIViewController {
    ...
    static func show(superV: UIViewController) {
        let vc = JXMyLuckyCodeViewController()
    ...
    override func viewDidLoad() {
        ...
        self.goodsImgV.layer.cornerRadius = self.goodsImgV.frame.height * 0.5
        self.goodsImgV.layer.masksToBounds = true

        self.goodsName.font = UIFont.zx_bodyFont
        self.goodsName.textColor = UIColor.white

        self.tabView.register(UINib(nibName: JXLuckyGoodsCodeCell.NibName, bundle: nil), forCellReuseIdentifier: JXLuckyGoodsCodeCell.reuseIdentifier)
...
extension JXMyLuckyCodeViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: JXLuckyGoodsCodeCell = tableView.dequeueReusableCell(withIdentifier: JXLuckyGoodsCodeCell.reuseIdentifier, for: indexPath) as! JXLuckyGoodsCodeCell
//            cell.delegate = self
...
extension JXMyLuckyCodeViewController: UITableViewDelegate {
...
extension JXMyLuckyCodeViewController {
    func jx_myCode() {
        ZXHUD.showLoading(in: self.view, text: ZX_LOADING_TEXT, delay: 0)
        JXCJModelView.jx_goodsCode(url: ZXAPIConst.Bet.myCode, pageNum: self.currentPage, pageSize: ZX.PageSize, turnCode: self.turnCode) { c, s, modle, msg in
            ...
            if s {
                ...
                if let mod = self.goodsCode {
                    self.goodsImgV.kf.setImage(with: URL(string: mod.memberImg))
                    self.goodsName.text = mod.phoneNumber
                }
